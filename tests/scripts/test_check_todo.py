...
class TestFindActiveChecklist:
    ...
    def test_find_existing_checklist(self, temp_project_root):
        ...
        checklist_path = temp_project_root / "docs" / "rules" / "checklists" / "build-checklist.md"
...
class TestCheckUncheckedItems:
    ...
    def test_find_unchecked_items(self, temp_project_root, sample_checklist_with_unchecked):
...
class TestMainFunction:
    ...
    def test_main_with_unchecked_items_non_strict(self, temp_project_root, sample_active_session, sample_checklist_with_unchecked):
        """Test main function with unchecked items in non-strict mode."""
        # Setup test files
        from tests.conftest import create_test_files, create_test_checklist
        create_test_files(temp_project_root, {}, sample_active_session)
        create_test_checklist(temp_project_root, "build", sample_checklist_with_unchecked)
        
        with patch('sys.argv', ['check_todo.py', '--project-root', str(temp_project_root)]):
            with patch('sys.exit') as mock_exit:
                main()
                mock_exit.assert_not_called()
    
    def test_main_with_unchecked_items_strict(self, temp_project_root, sample_active_session, sample_checklist_with_unchecked):
        ...
        with patch('sys.argv', ['check_todo.py', '--project-root', str(temp_project_root), '--strict']):
            with pytest.raises(SystemExit) as exc_info:
                main()
            
            assert exc_info.value.code == 1
    
    def test_main_with_all_checked(self, temp_project_root, sample_active_session, sample_checklist_all_checked):
...
```"""
    ...
    def test_main_with_custom_project_root(self, temp_project_root, sample_active_session, sample_checklist_all_checked):
