...
class TestMainFunction:
    ...
    def test_main_json_migration(self, temp_project_root, sample_legacy_json):
        ...
        with patch('sys.argv', ['migrate_session.py', '--project-root', str(temp_project_root), '--legacy-file', str(legacy_file)]):
    ...
    def test_main_yaml_frontmatter_migration(self, temp_project_root, sample_legacy_yaml_frontmatter):
    ...
    def test_main_dry_run(self, temp_project_root, sample_legacy_json):
        ...
        with patch('sys.argv', ['migrate_session.py', '--project-root', str(temp_project_root), '--legacy-file', str(legacy_file), '--dry-run']):
            with patch('sys.exit') as mock_exit:
                main()
                mock_exit.assert_not_called()
    ...
    def test_main_with_custom_output_file(self, temp_project_root, sample_legacy_json):
        ...
        with patch('sys.argv', ['migrate_session.py', '--project-root', str(temp_project_root), '--legacy-file', str(legacy_file), '--output-file', str(output_file)]):
            with patch('sys.exit') as mock_exit:
                main()
                mock_exit.assert_not_called()
    ...
    def test_main_with_backup(self, temp_project_root, sample_legacy_json):
        ...
        with patch('sys.argv', ['migrate_session.py', '--project-root', str(temp_project_root), '--legacy-file', str(legacy_file), '--backup']):
            with patch('sys.exit') as mock_exit:
                main()
                mock_exit.assert_not_called()
        
        # Check that backup was created
        backup_path = temp_project_root / "docs" / "session-context" / "ACTIVE_SESSION.md.backup"
...
class TestErrorHandling:
    ...
    def test_unicode_handling(self, temp_project_root):
        ...
        legacy_data = {
            ...
            "agent": "conductor",
            "task": {
                "title": "Task with unicode: αβγ δεζ 中文 🚀"
            },
