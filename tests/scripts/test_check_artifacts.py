...
class TestLoadActiveSession:
    ...
    def test_load_existing_session(self, temp_project_root, sample_active_session):
        ...
        session_path = temp_project_root / "docs" / "session-context" / "ACTIVE_SESSION.md"
...
class TestGetPhaseArtifacts:
    ...
    def test_get_phase_with_no_artifacts(self, sample_pipeline_config):
        ...
        sample_pipeline_config["phases"].append({
            "name": "no_artifacts",
            "description": "Phase with no artifacts"
        })
...
class TestResolveArtifactPaths:
    ...
    def test_resolve_absolute_paths(self, temp_project_root, sample_pipeline_config):
        ...
        result = resolve_artifact_paths(temp_project_root, artifacts, sample_pipeline_config)
        
        assert len(result) == 2
        assert result[0] == temp_project_root / "docs" / "test.md"
        assert result[1] == temp_project_root / "config" / "test.yml"
    
    def test_resolve_documentation_files(self, temp_project_root, sample_pipeline_config):
...
class TestMainFunction:
    ...
    def test_main_with_existing_artifacts(self, temp_project_root, sample_pipeline_config, sample_active_session):
        """Test main function with all artifacts existing."""
        # Setup test files
        from tests.conftest import create_test_files, create_test_artifacts
        create_test_files(temp_project_root, sample_pipeline_config, sample_active_session)
        create_test_artifacts(temp_project_root, ["setup.md", "config.yml"])
        
        with patch('sys.argv', ['check_artifacts.py', '--project-root', str(temp_project_root)]):
            with patch('sys.exit') as mock_exit:
                main()
                mock_exit.assert_not_called()
    
    def test_main_with_missing_artifacts_non_strict(self, temp_project_root, sample_pipeline_config, sample_active_session):
    ...
    def test_main_with_missing_artifacts_strict(self, temp_project_root, sample_pipeline_config, sample_active_session):
        """Test main function with missing artifacts in strict mode."""
        # Setup test files (without artifacts)
        from tests.conftest import create_test_files
        create_test_files(temp_project_root, sample_pipeline_config, sample_active_session)
        
        with patch('sys.argv', ['check_artifacts.py', '--project-root', str(temp_project_root), '--strict']):
            with pytest.raises(SystemExit) as exc_info:
                main()
            
            assert exc_info.value.code == 1
    
    def test_main_with_create_missing_flag(self, temp_project_root, sample_pipeline_config, sample_active_session):
        """Test main function with create-missing flag."""
        # Setup test files (without artifacts)
        from tests.conftest import create_test_files
        create_test_files(temp_project_root, sample_pipeline_config, sample_active_session)
        
        with patch('sys.argv', ['check_artifacts.py', '--project-root', str(temp_project_root), '--create-missing']):
            with patch('sys.exit') as mock_exit:
                main()
                mock_exit.assert_not_called()
    ...
    def test_main_with_custom_phase(self, temp_project_root, sample_pipeline_config, sample_active_session):
        ...
        with patch('sys.argv', ['check_artifacts.py', '--project-root', str(temp_project_root), '--phase', 'discovery']):
            with patch('sys.exit') as mock_exit:
                main()
                mock_exit.assert_not_called()
    
    def test_main_with_no_artifacts_required(self, temp_project_root, sample_pipeline_config, sample_active_session):
    ...
    def test_main_missing_phase_in_session(self, temp_project_root, sample_pipeline_config):
...
class TestErrorHandling:
    ...
    def test_artifact_path_resolution_edge_cases(self, temp_project_root, sample_pipeline_config):
        ...
        artifacts = ["file with spaces.md", "file-with-dashes.py", "file_with_underscores.yml"]
        result = resolve_artifact_paths(temp_project_root, artifacts, sample_pipeline_config)
        assert len(result) == 3
