...
def resolve_artifact_paths(project_root: Path, artifacts: List[str], pipeline_config: dict) -> List[Path]:
    ...
    for artifact in artifacts:
        ...
        if artifact.startswith('/'):
            ...
            # Documentation files
            if 'opportunity' in artifact or 'use-case' in artifact or 'ai-readiness' in artifact:
                base_path = artifact_paths.get('preengagement', {}).get('base_path', '/preengagement')
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Check for missing artifacts in current phase")
    ...
    artifact_paths = resolve_artifact_paths(args.project_root, required_artifacts, pipeline_config)
    ...
    if missing_artifacts:
        ...
        if args.create_missing:
            ...
            for artifact_path in missing_artifacts:
                ...
                with open(artifact_path, 'w') as f:
                    ...
                    f.write(f"<!-- Placeholder file created by check_artifacts.py -->\n")
                    f.write(f"<!-- TODO: Add actual content for {artifact_path.name} -->\n")
        ...
        if args.strict and not args.create_missing:
            ...
            logger.warning("Missing artifacts found but not failing CI (use --strict to fail)")
    else:
