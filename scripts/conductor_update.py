...
def load_active_session(project_root: Path) -> Tuple[dict, str]:
    ...
    active_session_path = project_root / "docs" / "session-context" / "ACTIVE_SESSION.md"
...
def update_session_phase(project_root: Path, content: str, sections: dict, next_phase: str, next_phase_info: dict) -> str:
    ...
    try:
        ...
        if session_notes_yaml:
            ...
            session_notes['decisions'].append(f"Advanced to {next_phase} phase via conductor automation")
            ...
            session_notes['next_steps'] = [
                ...
                f"Update task assignments for new phase"
            ]
        ...
        current_work_str = yaml.dump(current_work, default_flow_style=False, sort_keys=False)
        new_content = replace_yaml_section(new_content, 'Current Work', current_work_str)
        
        # Replace Session Metadata section
        if session_metadata_yaml:
            session_metadata_str = yaml.dump(session_metadata, default_flow_style=False, sort_keys=False)
            new_content = replace_yaml_section(new_content, 'Session Metadata', session_metadata_str)
        
        # Replace Session Notes section
        if session_notes_yaml:
            session_notes_str = yaml.dump(session_notes, default_flow_style=False, sort_keys=False)
            new_content = replace_yaml_section(new_content, 'Session Notes', session_notes_str)
...
def main():
    ...
    if args.force_phase:
        ...
        if not next_phase:
            logger.info(f"No next phase found for '{current_phase}' - pipeline complete!")
            return
    ...
    new_content = update_session_phase(args.project_root, content, sections, next_phase, next_phase_info)
    
    if args.dry_run:
        logger.info("DRY RUN - Would update ACTIVE_SESSION.md with new phase information")
