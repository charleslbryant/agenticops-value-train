...
def main():
    ...
    parser = argparse.ArgumentParser(description="Check for unchecked todo items in active checklist")
    ...
    if unchecked_items:
        logger.error(f"Found {len(unchecked_items)} unchecked items in {checklist_path.name}:")
        ...
        if args.strict:
            ...
            logger.warning("Unchecked items found but not failing CI (use --strict to fail)")
    else:
